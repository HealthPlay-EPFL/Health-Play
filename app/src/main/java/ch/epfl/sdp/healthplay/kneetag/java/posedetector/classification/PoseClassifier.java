...
import static ch.epfl.sdp.healthplay.kneetag.java.posedetector.classification.Utils.maxAbs;
import static ch.epfl.sdp.healthplay.kneetag.java.posedetector.classification.Utils.multiply;
...
public class PoseClassifier {
  ...
  public ClassificationResult classify(List<PointF3D> landmarks) {
    ...
    for (PoseSample poseSample : poseSamples) {
      ...
      for (int i = 0; i < embedding.size(); i++) {
        originalMax =
            max(
                originalMax,
                    maxAbs(multiply(Utils.subtract(embedding.get(i), sampleEmbedding.get(i)), axesWeights)));
        flippedMax =
            max(
                flippedMax,
                maxAbs(
    ...
    for (Pair<PoseSample, Float> sampleDistances : maxDistances) {
      ...
      float flippedSum = 0;
      for (int i = 0; i < embedding.size(); i++) {
        originalSum += Utils.sumAbs(multiply(
                Utils.subtract(embedding.get(i), sampleEmbedding.get(i)), axesWeights));
        flippedSum += Utils.sumAbs(
            multiply(Utils.subtract(flippedEmbedding.get(i), sampleEmbedding.get(i)), axesWeights));
      }
